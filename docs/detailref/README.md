...
当事件中存在发送者昵称时，返回发送者昵称。

当事件中不存在发送者昵称但是存在发送者名片时，返回发送者名片。

当事件中既不存在发送者昵称也不存在发送者名片时，此命令会自动进行查询，可能需要平台有相关API支持，查询失败返回空文本。如果查询成功，会将其结果缓存起来，下次不再查询。
