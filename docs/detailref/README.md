...
读一个以UTF-8编码的INI文件下某节的内容，返回的是一个对象，包含此节下的所有键值对，若文件不存在或者无对应节，返回空对象。若已有文件解析失败则抛出异常。
...
bbb=2
```
代码：`【转文本【读配置键【应用目录】xxx.ini@kkk】】`将返回`{"aaa":"1","bbb":"2","ccc":"3"}`。
...
```text
【转文本【正则@a1b2c3@\\d】】 ## 得到 [["1"],["2"],["3"]]
【正则@a1b2c3@\\d@1@0】 ## 得到 2
【转文本【正则@a1b2c3@\\d@*@0】】 ## 得到 ["1","2","3"]
【转文本【正则@a1b2c3@\\d@1】】 ## 得到 ["2"]
```
...
【BR解码@<font color="red">字节集</font>】

将BR编码的字节集解码，返回解码后的字节集。

如果解码出错，返回空字节集。

注意，只有BR编码的字节集才能进行BR解码。
...
注意此命令无法在`框架初始化`中使用，因为初始化的时候是不允许脚本并发执行的。
...
【图片蒙版@<font color="red">大图片字节集</font>@<font color="red">小图片字节集</font>@<font color="red">x</font>@<font color="red">y</font>】

以小图片为蒙版层，对大图片的指定位置进行蒙版。规则：大图片透明度 = 小图片灰度值
...
- `contrast = 1.0`：保持原始对比度
- `contrast > 1.0`（如1.5、2.0）：降低对比度，线条更柔和
- `contrast < 1.0`（如0.5、0.8）：增强对比度，线条更清晰，背景更黑

线稿图片生成原理：

1：原图生成灰度图（算法同`【图片变灰】`命令）和反向灰度图

2：对反向灰度图做图片模糊（算法同`【图片模糊】`命令）得到遮罩图

3：颜色减淡：线稿图 = 灰度图 / (255 - 遮罩图) * 256

4：对比度调整：使用幂函数 `new_val = val^(1/contrast)` 调整对比度
...
【图片线稿2@<font color="red">图片字节集</font>@<font color="red">kernel_size</font>@<font color="red">contrast</font>】

将图片变成线稿图片（黑底白线），<font color="red">kernel_size</font>为最小值滤镜的核大小，必须是大于0的奇数（如3、5、7等），否则抛出异常。<font color="red">contrast</font>为对比度调整参数，必须大于等于0，可以是小数，可以省略，默认1.0。
...
2：对反向灰度图应用最小值滤镜得到遮罩图（在kernel_size × kernel_size的邻域内取最小值）
...
【图片反色@<font color="red">图片字节集</font>】

将图片的每个通道都用255来减，透明度通道除外。
...
【截屏@<font color="red">文本</font>】

返回第<font color="red">n</font>(n从0开始数，可省略，默认为0)个显示器的屏幕截图。若没有显示器，则返回空字节集
...
此命令用于调用lua。在<font color="red">脚本内容</font>中，可以使用如下内置函数：
...
**注意**:TTS的合成是非常昂贵的，目前白嫖了微软的TTS，但是微软已经开始上强度了，准备跑路吧。
...
【计数@<font color="red">内容</font>@<font color="red">元素</font>】

计算<font color="red">内容</font>中<font color="red">元素</font>出现的次数。

内容支持文本、字节集、数组。

当内容类型为文本时，元素类型必须为文本。

当内容类型为字节集时，元素类型必须为字节集。
...
**注意**，在以下情况下，重启命令可能会导致资源泄露：

1：【网页截图】命令正在运行的时候，chrome实例可能不会自动退出

2：有运行时间超过5s的脚本的时候，会在5s时强制中断脚本

3：linux下

4：红色问答本身被其它进程管理器控制的时候

5：其它会导致资源泄露的情况

**注意**：这是一个实验性质的极度不稳定命令，用法很有可能会在之后的版本中发生变化(可能删除)
...
【取头像@<font color="red">用户ID</font>】

返回<font color="red">用户ID</font>的头像url，<font color="red">用户ID</font>可以省略，默认是【发送者ID】。

如果无法获取头像，则返回空文本。注意，这个命令可能返回非公网的url。
...
第二种返回目标用户ID在当前群聊的历史消息ID数组，从新到旧排序（因内存条很贵，所以最多返回100条数据）。


### 群历史消息

【群历史消息@<font color="red">数量</font>】

获取<font color="red">数量</font>条当前群的历史消息的消息ID，最多100个，从新到旧。
...
发送一条消息，然后返回消息ID，注意，输出流不支持【分页】。如果消息发送失败，则返回空文本。


### 发送私聊


【发送私聊@<font color="red">内容</font>@<font color="red">目标用户ID</font>】

在群聊中使用，向<font color="red">目标用户ID</font>发送一条私聊消息，然后返回消息ID，注意，发送私聊不支持【分页】。如果消息发送失败，则返回空文本。

<font color="red">目标用户ID</font>可以省略，默认向【发送者ID】发送。

如果没有在群聊中使用此命令，则不做任何事，并且直接返回空文本。
...
返回当前邮件的主题。仅在邮件平台（email）下有效，其它平台返回空文本。
...
【表情回应@<font color="red">emoji表情</font>@<font color="red">消息ID</font>】

对当前消息进行表情回应，目前仅支持部分emoji。例子：`【表情回应@😘】`

<font color="red">消息ID</font>是可以省略的，默认是当前消息。
...
返回当前群的下一条消息、其发送者、消息ID(一个RedLang对象)。

返回示例:`{"发送者ID":"1875159423","消息":"你好，世界","消息ID":"xxxxxxx"}`
...
【重定向@<font color="red">脚本名</font>】

将当前脚本重定向到另一个脚本中进行处理。平台相关的数据(包括子关键词)将会被复制，序号、变量等其它数据不会保留。

通过这个命令，可以使得一个脚本处理多个关键词，而无须过度使用正则匹配。

建议两个脚本的触发方式和匹配方式相同或相似。
...
【设置桌面背景@<font color="red">图片路径</font>】

<font color="red">图片路径</font>只可以是本地图片路径。
...
可以点击webui上的`其它`->`全局初始化器`按钮，来设置针对所有包的初始化脚本器。在包初始化之前，会执行全局初始化器。在保存全局初始化脚本的时候，会为每个包执行全局初始化脚本。
