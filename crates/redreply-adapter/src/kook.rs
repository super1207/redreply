...
impl KookConnect {
    ...
    async fn http_get_json_t(
        &self,
        uri: &str,
        use_cache: bool,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
        lazy_static! {
            static ref CACHE: std::sync::RwLock<VecDeque<(String, serde_json::Value, u64)>> =
                std::sync::RwLock::new(VecDeque::from([]));
        }
    ...
    async fn get_gateway(&self) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        let ret_json = self
            .http_get_json(&format!("/gateway/index?compress=1"))
            .await?;
        Ok(ret_json
            .get("url")
            .ok_or("get url err")?
    ...
    async fn send_private_msg(
        ...
        let msg_id = ret_json
            .get("msg_id")
            .ok_or("msg_id not found")?
            .as_str()
            .ok_or("msg_id not str")?;
        Ok(msg_id.to_owned())
    }
    ...
    async fn get_guild_member_info(
        &self,
        guild_id: &str,
        user_id: &str,
        use_cache: bool,
    ) -> Result<GroupMemberInfo, Box<dyn std::error::Error + Send + Sync>> {
        let stranger_info = self
            .http_get_json_t(
                &format!("/user/view?user_id={user_id}&guild_id={guild_id}"),
                use_cache,
            )
            .await?;
        let guild_info = self
            .http_get_json_t(&format!("/guild/view?guild_id={guild_id}"), use_cache)
            .await?;
        let owner_id = guild_info
            .get("user_id")
            .ok_or("get user_id err")?
            .as_str()
            .ok_or("user_id not str")?;
        let role;
        if owner_id == user_id {
            role = "owner";
        } else {
            let roles = stranger_info
                .get("roles")
                .ok_or("get roles err")?
                .as_array()
                .ok_or("roles not arr")?;
        ...
        Ok(GroupMemberInfo {
            ...
            nickname: stranger_info
                .get("username")
                .ok_or("get username err")?
                .as_str()
                .ok_or("username not str")?
                .to_owned(),
            card: stranger_info
                .get("nickname")
                .ok_or("get nickname err")?
                .as_str()
                .ok_or("nickname not str")?
                .to_owned(),
            sex: "unknown".to_owned(),
            age: 0,
            area: "".to_owned(),
            join_time: (stranger_info
                .get("joined_at")
                .ok_or("get joined_at err")?
                .as_u64()
                .ok_or("joined_at not u64")?
                / 1000) as i32,
            last_sent_time: (stranger_info
                .get("active_time")
                .ok_or("get active_time err")?
                .as_u64()
                .ok_or("active_time not u64")?
                / 1000) as i32,
            level: "0".to_owned(),
            role: role.to_owned(),
            unfriendly: false,
            ...
            avatar: stranger_info
                .get("avatar")
                .ok_or("avatar not found")?
                .as_str()
                .ok_or("avatar not str")?
                .to_owned(),
        })
    }
    ...
    async fn deal_private_message_event(
        &self,
                ...
                if tp == "header" {
                    ...
                    if is_group {
                        let group_id = data
                            ...
                            .as_str()
                            .ok_or("target_id not str")?;
                        let user_id = data
                            .get("author_id")
                            .ok_or("author_id not found")?
                            .as_str()
                            .ok_or("author_id not str")?;
    ...
    async fn deal_group_message_event(
        &self,
        data: &serde_json::Value,
        user_id: &str,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let group_info = self
            .http_get_json_t(&format!("/channel/view?target_id={group_id}"), true)
            .await?;
        let guild_id = group_info
            .get("guild_id")
            .ok_or("get guild_id err")?
            .as_str()
            .ok_or("guild_id not str")?;

        // 获取发送者
        let mut sender: GroupMemberInfo =
            self.get_guild_member_info(guild_id, user_id, true).await?;
    ...
    // }
    async fn deal_group_increase_event(
        &self,
        data: &serde_json::Value,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let guild_id = data
    ...
    async fn deal_group_decrease_event(
        &self,
        data: &serde_json::Value,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let guild_id = data
    ...
    async fn deal_group_recall(
        ...
        let group_id = data
            ...
            .ok_or("body not found")?
            .get("channel_id")
            .ok_or("channel_id not found")?
            .as_str()
            .ok_or("channel_id not str")?;
        let group_info = self
            .http_get_json_t(&format!("/channel/view?target_id={group_id}"), true)
            .await?;
        let guild_id = group_info
    ...
    pub async fn get_login_info(
        &self,
    ) -> Result<LoginInfo, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let user_id = login_info
            ...
            .as_str()
            .ok_or("id not str")?;
        let nickname = login_info
            .get("username")
            .ok_or("get username err")?
            .as_str()
            .ok_or("username not str")?;
        let avatar = login_info
            .get("avatar")
            .ok_or("get avatar err")?
            .as_str()
            .ok_or("avatar not str")?;
        Ok(LoginInfo {
            user_id: user_id.to_owned(),
    ...
    async fn deal_kook_event(
        &self,
        data: serde_json::Value,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let tp = data
            .get("channel_type")
            .ok_or("channel_type not found")?
            .as_str()
            .ok_or("channel_type not str")?;
        if tp == "GROUP" {
    ...
    async fn upload_asset(
        ...
        let form = reqwest::multipart::Form::new().part(
            "file",
            reqwest::multipart::Part::bytes(file_bin).file_name("test"),
        );
    ...
    async fn make_kook_msg(
        &self,
        message_arr: &serde_json::Value,
        is_group: bool,
    ) -> Result<(Vec<(i32, String)>, String), Box<dyn std::error::Error + Send + Sync>> {
        ...
        for it in message_arr.as_array().ok_or("message not arr")? {
            ...
            if tp == "text" {
                ...
                if music_type == "custom" {
                    ...
                    header.insert(
                        "User-Agent".to_owned(),
                        "https://github.com/super1207/KookOneBot".to_owned(),
                    );
                    let ret = Self::http_post(&url, vec![], &header, false).await?;
                    let ret_json: serde_json::Value =
                        serde_json::from_str(&String::from_utf8(ret)?)?;
                    ...
                    lazy_static! {
                        static ref AT_REGEX: Regex = Regex::new(r"://(.+)/amobile").unwrap();
                    }
                    audio = AT_REGEX
                        .replace_all(&audio, "://aqqmusic.tc.qq.com/amobile")
                        .to_string();
                    let title = read_json_str(music_data, "title");
                    let image = read_json_str(music_data, "pic");
                    let js = serde_json::json!([{
                ...
                let s = json_to_cq_str(&j).map_err(|x| format!("json_to_cq_str err:{:?}", x))?;
    ...
    async fn get_stranger_info(
        ...
        user_id: &str,
        use_cache: bool,
    ) -> Result<StrangerInfo, Box<dyn std::error::Error + Send + Sync>> {
        let stranger_info = self
            .http_get_json_t(&format!("/user/view?user_id={user_id}"), use_cache)
            .await?;
        let user_id = stranger_info
            .get("id")
            .ok_or("get id err")?
            .as_str()
            .ok_or("id not str")?;
        let nickname = stranger_info
            .get("username")
            .ok_or("get username err")?
            .as_str()
            .ok_or("username not str")?;
    ...
    async fn deal_ob_get_stranger_info(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_group_info(
        ...
        group_id: &str,
        use_cache: bool,
    ) -> Result<GroupInfo, Box<dyn std::error::Error + Send + Sync>> {
        let stranger_info = self
            .http_get_json_t(&format!("/channel/view?target_id={group_id}"), use_cache)
            .await?;
        let group_id = stranger_info
            .get("id")
            .ok_or("get id err")?
            .as_str()
            .ok_or("id not str")?;
        let group_name = stranger_info
            .get("name")
            .ok_or("get name err")?
            .as_str()
            .ok_or("name not str")?;
    ...
    async fn deal_ob_get_group_info(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_group_list(
        ...
        let ret_json = self
            .http_get_json_t(&format!("/channel/list?guild_id={guild_id}"), false)
            .await?;
        let channel_arr = ret_json
            .get("items")
            .ok_or("get items err")?
            .as_array()
            .ok_or("items not arr")?;
    ...
    async fn deal_ob_get_group_list(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn deal_ob_get_group_member_info(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let use_cache = !Self::get_json_bool(params, "no_cache");
        let info = self
            .get_group_member_info(&group_id, &user_id, use_cache)
            .await?;
        let send_json = serde_json::json!({
            "status":"ok",
            "retcode":0,
    ...
    async fn deal_ob_set_group_kick(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn deal_ob_set_group_leave(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn deal_ob_set_group_name(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn deal_ob_set_group_card(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    pub async fn get_friend_list(
        &self,
    ) -> Result<Vec<FriendInfo>, Box<dyn std::error::Error + Send + Sync>> {
        let mut ret_vec = vec![];
        let friend_list = self
            .http_get_json_t(&format!("/user-chat/list"), false)
            .await?;
            ...
            .ok_or("items not arr")?
        {
            ...
            let id = target_info
                ...
                .as_str()
                .ok_or("id not str")?;
            let username = target_info
                .get("username")
                .ok_or("username not found")?
                .as_str()
                .ok_or("username not str")?;
            let avatar = target_info
                .get("avatar")
                .ok_or("avatar not found")?
                .as_str()
                .ok_or("avatar not str")?;
        ...
        let page_total = meta
            .get("page_total")
            .ok_or("page_total not found")?
            .as_i64()
            .ok_or("page_total not i32")?;
        for page in 1..page_total {
            let friend_list = self
                .http_get_json_t(&format!("/user-chat/list?page={page}"), false)
                .await?;
            for it in friend_list
                .get("items")
                .ok_or("items not found")?
    ...
    async fn deal_ob_get_friend_list(
        &self,
        _params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_group_member_list(
        &self,
        group_id: &str,
    ) -> Result<Vec<GroupMemberInfo>, Box<dyn std::error::Error + Send + Sync>> {
        let group_info = self
            .http_get_json_t(&format!("/channel/view?target_id={group_id}"), true)
            .await?;
        let guild_id = group_info
        ...
        let ret_json = self
            .http_get_json_t(&format!("/guild/user-list?guild_id={guild_id}"), false)
            .await?;
        let items = ret_json
            .get("items")
            .ok_or("get items err")?
        ...
        for page in 1..page_total {
            let ret_json = self
                .http_get_json_t(
                    &format!("/guild/user-list?guild_id={guild_id}&page={page}"),
                    false,
                )
                .await?;
            for it in ret_json
    ...
    async fn deal_ob_get_group_member_list(
        &self,
        params: &serde_json::Value,
        echo: &serde_json::Value,
    ) -> Result<serde_json::Value, Box<dyn std::error::Error + Send + Sync>> {
...
impl BotConnectTrait for KookConnect {
    async fn disconnect(&mut self) {
        ...
        if self.stop_tx.is_some() {
            let _foo = self
                ...
                .clone()
                .unwrap()
                .send_timeout(true, Duration::from_secs(1))
                .await;
        }
    }
    ...
    async fn connect(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.token = config_json
            .get("Token")
            .ok_or("kook url格式错误:没有Token")?
            .as_str()
            .ok_or("kook url格式错误:Token不是字符串")?
            .to_owned();
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if index == 30 {
                    ...
                    let foo = write_halt
                        .send(tungstenite::Message::Text(json_str.into()))
                        .await;
                    if foo.is_err() {
    ...
    async fn call_api(
        ...
        let send_json = match action {
            ...
            "get_stranger_info" => self.deal_ob_get_stranger_info(&params, &echo).await?,
            "get_group_info" => self.deal_ob_get_group_info(&params, &echo).await?,
            "get_group_list" => self.deal_ob_get_group_list(&params, &echo).await?,
            "get_group_member_info" => self.deal_ob_get_group_member_info(&params, &echo).await?,
            "set_group_kick" => self.deal_ob_set_group_kick(&params, &echo).await?,
            "set_group_leave" => self.deal_ob_set_group_leave(&params, &echo).await?,
            "set_group_name" => self.deal_ob_set_group_name(&params, &echo).await?,
            "set_group_card" => self.deal_ob_set_group_card(&params, &echo).await?,
            "get_friend_list" => self.deal_ob_get_friend_list(&params, &echo).await?,
            "get_group_member_list" => self.deal_ob_get_group_member_list(&params, &echo).await?,
            "get_cookies" => self.deal_ob_get_cookies(&params, &echo).await?,
            "can_send_image" => {
                serde_json::json!({
                    "status":"ok",
                    "retcode":0,
