    ...
    async fn connect(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...

        {
            ...
            let bot_arr = myself
                ...
                .as_array()
                .ok_or("data not array")?;
            for bot in bot_arr {
                let platform = bot
                    .get("platform")
                    .ok_or("platform not found")?
                    .as_str()
                    .ok_or("platform not str")?;
                let bot_id = bot
                    .get("bot_id")
                    .ok_or("bot_id not found")?
                    .as_str()
                    .ok_or("bot_id not str")?;
                self.platforms
                    .write()
                    .unwrap()
                    .push((platform.to_string(), bot_id.to_string()));
            }
        }
        ...
        if let Some(access_token) = mp.get("access_token") {
            request.headers_mut().insert(
                "Authorization",
                tungstenite::http::HeaderValue::from_str(&format!("Bearer {}", access_token))
                    .unwrap(),
            );
        }
