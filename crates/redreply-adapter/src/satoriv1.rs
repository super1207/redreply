...
use crate::{
    cqapi::cq_add_log_w,
    mytool::{
        cq_params_encode, cq_text_encode, read_json_obj, read_json_obj_or_null, read_json_str,
        str_msg_to_arr,
    },
};
...
async fn http_post(
    ...
    req.headers_mut().append(
        reqwest::header::HeaderName::from_str("X-Platform")?,
        reqwest::header::HeaderValue::from_str(platform)?,
    );
    req.headers_mut().append(
        reqwest::header::HeaderName::from_str("X-Self-ID")?,
        reqwest::header::HeaderValue::from_str(self_id)?,
    );
    req.headers_mut().append(
...
fn pre_parse_satori_msg(
    html: &str,
) -> Result<Vec<SatoriNode>, Box<dyn std::error::Error + Send + Sync>> {
    let mut to_ret = vec![];
    let mut deep = 0;
...
impl Satoriv1Connect {
            ...
            if child.tag_name == "at" {
                ...
                if platform == "llonebot" {
                    let face_id = child
                        .attr_map
                        .get("id")
                        .ok_or("No id at llonebot face element")?;
        ...
        for it in arr {
            ...
            if tp == "text" {
                ...
                if file.starts_with("http://") || file.starts_with("https://") {
                    ...
                    out += &format!(
                        "<img src={} />",
                        serde_json::json!("data:image/png;base64,".to_owned() + b64)
                    );
                }
            } else if tp == "reply" {
                let id = it
                ...
                let t = id
                    .split("b73d7536-d8fa-4dda-b194-4acc51898a91")
                    .collect::<Vec<&str>>();
                ...
                if file.starts_with("http://") || file.starts_with("https://") {
                    ...
                    out += &format!(
                        "<audio src={} />",
                        serde_json::json!("data:audio/mp3;base64,".to_owned() + b64)
                    );
                }
            } else if tp == "poke" {
    ...
    async fn conv_event(
        json_data: serde_json::Value,
        platforms: std::sync::Weak<std::sync::RwLock<Vec<(String, String)>>>,
        user_channel_map: std::sync::Weak<
            std::sync::RwLock<std::collections::HashMap<String, String>>,
        >,
        group_groups_map: std::sync::Weak<
            std::sync::RwLock<std::collections::HashMap<String, String>>,
        >,
    ) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        let op = read_json_str(&json_data, "op");
        if op != "2" {
            crate::cqapi::cq_add_log(format!("SATORI收到数据:{}", json_data.to_string()).as_str())
                .unwrap();
        }
        if op == "2" {
            // 心跳回复
            ...
            for item in logins {
                ...
                let self_id = item["user"]["id"]
                    .as_str()
                    .ok_or("login中不存在user-id字段")?;
            ...
            if type_t == "login-removed" || type_t == "login-added" || type_t == "login-updated" {
                ...
                if type_t == "login-removed" {
                    if index != usize::MAX {
                        lk.remove(index);
                        cq_add_log_w(&format!("remove account:`{}`:`{}`", platform, self_id))
                            .unwrap();
                    }
                } else {
                    if index == usize::MAX {
                ...
                if guild_opt.is_some() {
                    ...
                    group_groups_map
                        .upgrade()
                        .ok_or("upgrade group_groups_map失败")?
                        .write()
                        .unwrap()
                        .insert(key, guild_id.to_owned());
                        ...
                        for child in childen {
                            if child.tag_name == "chronocat:poke" {
                                let id_str = child
                                    .attr_map
                                    .get("user-id")
                                    .ok_or("No user-id at chronocat:poke element")?;
                                let id = html_escape::decode_html_entities(&id_str);
                                let oid_str = child
                                    .attr_map
                                    .get("operator-id")
                                    .ok_or("No operator-id at chronocat:poke element")?;
                                ...
                                tokio::task::spawn_blocking(move || {
                                    if let Err(e) =
                                        crate::cqevent::do_1207_event(&event_json.to_string())
                                    {
                                        crate::cqapi::cq_add_log(format!("{:?}", e).as_str())
                                            .unwrap();
                                    }
                                });
                            }
                    ...
                    user_channel_map
                        .upgrade()
                        .ok_or("upgrade user_channel_map失败")?
                        .write()
                        .unwrap()
                        .insert(key, channel_id.clone());
                    let event_json = serde_json::json!({
                        "time":tm,
                        "self_id":self_id,
                        "platform":platform,
                ...
                let message_id =
                    format!("{raw_message_id}b73d7536-d8fa-4dda-b194-4acc51898a91{channel_id}");
    ...
    async fn send_group_msg(
        ...
        if message.is_array() {
            ...
            if passive_id != "" {
                satori_content = format!(
                    "<passive id={} />{}",
                    serde_json::json!(passive_id),
                    satori_content
                )
            }
        ...
        // 处理日志
        {
            ...
            if out_str.is_some() {
                crate::cqapi::cq_add_log(
                    format!(
                        "发送数据(platform:{platform},self_id:{self_id}):{}...",
                        out_str.unwrap()
                    )
                    .as_str(),
                )
                ...
                crate::cqapi::cq_add_log(
                    format!("发送数据(platform:{platform},self_id:{self_id}):{}", js_str).as_str(),
                )
                .unwrap();
            }
        }
        ...
        let ret = http_post(
            &format!("{}/message.create", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
    ...
    async fn send_private_msg(
        ...
        let key = format!("{platform} {self_id} {user_id}");
        let channel_id = self_t
            .user_channel_map
            .read()
            .unwrap()
            .get(&key)
            .ok_or("user_id not match any channel")?
            .to_owned();
        let message = params.get("message").ok_or("message is not exist")?;
        let to_send;
        if message.is_array() {
    ...
    async fn get_login_info(
        ...
        let ret = http_post(
            &format!("{}/login.get", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
    ...
    async fn get_group_list(
        ...
        let ret = http_post(
            &format!("{}/channel.list", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
        ...
        let channel_list = ret
            .get("data")
            .ok_or("data is not exist")?
            .as_array()
            .ok_or("data is not array")?;
        ...
        for channel in cl {
            let id = channel
                .get("id")
                .ok_or("id is not exist")?
                .as_str()
                .ok_or("id is not string")?;
            let key = format!("{platform} {self_id} {id}");
            self_t
                .group_groups_map
                .write()
                .unwrap()
                .insert(key, groups_id.to_owned());
    ...
    async fn get_group_member_info(
        ...
        let ret = http_post(
            &format!("{}/guild.member.get", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
    ...
    async fn get_stranger_info(
        ...
        .unwrap();
        let user = http_post(
            &format!("{}/user.get", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
    ...
    async fn delete_msg(
        ...
        http_post(
            &format!("{}/message.delete", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
    ...
    async fn get_msg(
        ...
        let satori_msg = http_post(
            &format!("{}/message.get", self_t.http_url),
            platform,
            self_id,
            &self_t.token,
            &to_send,
        ...
        let content = satori_msg["content"]
            .as_str()
            .ok_or("not found content in satori message")?;
...
impl BotConnectTrait for Satoriv1Connect {
    ...
    async fn connect(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let url_ws = config_json
            .get("uri")
            .ok_or("satori url格式错误:没有uri")?
            .as_str()
            .ok_or("satori url格式错误:uri不是字符串")?;
        let is_ssl = config_json
            .get("use_tls")
            .ok_or("satori url格式错误:没有use_tls")?
            .as_bool()
            .ok_or("satori url格式错误:use_tls不是bool")?;
        self.token = config_json
            .get("token")
            .ok_or("satori url格式错误:没有token")?
            .as_str()
            .ok_or("satori url格式错误:token不是字符串")?
            .to_owned();
        let ws_url;
        if is_ssl {
        ...
        let user_channel_map =
            Arc::<std::sync::RwLock<HashMap<String, String>>>::downgrade(&self.user_channel_map);
        let group_groups_map =
            Arc::<std::sync::RwLock<HashMap<String, String>>>::downgrade(&self.group_groups_map);
