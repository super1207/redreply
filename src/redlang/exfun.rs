...
use super::RedLang;
use crate::{
    cq_add_log_w, cqapi::cq_add_log, pyserver::call_py_block, redlang::get_random, RT_PTR,
};
use base64::{
    alphabet,
...
use image::{AnimationDecoder, EncodableLayout, ImageBuffer, Rgba};
use imageproc::geometric_transformations::Interpolation;
use imageproc::geometric_transformations::{rotate_about_center, warp_with, Projection};
use std::io::Cursor;
...
pub fn init_ex_fun_map() {
    ...
    add_fun(vec!["系统代理"], |_self_t, _params| {
        if cfg!(target_os = "windows") {
            ...
            let internet_setting: winreg::RegKey = hkcu
    ...
    add_fun(vec!["运行目录"], |_self_t, _params| {
        ...
        return Ok(Some(
            crate::mytool::deal_path_str(&exe_path_str).to_string(),
        ));
    });
    ...
    add_fun(vec!["正则"], |self_t, params| {
        ...
        return Ok(Some(
            self_t.build_arr(sub_key_vec.iter().map(AsRef::as_ref).collect()),
        ));
    });
    add_fun(vec!["转字节集"], |self_t, params| {
        ...
        if code == "" || code == "utf8" {
            ...
            str_vec = encoding::Encoding::encode(
                encoding::all::GBK,
                &text,
    ...
    add_fun(vec!["透视变换"], |self_t, params| {
        ...
        let img = ImageReader::new(Cursor::new(img_bin))
            .with_guessed_format()?
            .decode()?
            .to_rgba8();
        ...
        let dst = cv(dst_t)?;
        let src = cv(src_t)?;
        let p = Projection::from_control_points(src, dst)
            .ok_or("Could not compute projection matrix")?
            .invert();
        ...
        let img_out = image::imageops::crop(
            &mut img2,
            x_min as u32,
            y_min as u32,
            (x_max - x_min) as u32,
    ...
    add_fun(vec!["图片叠加", "图像叠加"], |self_t, params| {
        fn img_paste(
            ...
            let img1 = ImageReader::new(Cursor::new(img_vec_big))
                .with_guessed_format()?
                .decode()?
                .to_rgba8();
            let img2 = ImageReader::new(Cursor::new(img_vec_sub))
                .with_guessed_format()?
                .decode()?
                .to_rgba8();
            let w = img1.width();
    ...
    add_fun(vec!["图片覆盖", "图像覆盖"], |self_t, params| {
        ...
        let img_vec_big = RedLang::parse_bin(&text1)?;
        let img_vec_sub = RedLang::parse_bin(&text2)?;
        let mut img_big = ImageReader::new(Cursor::new(img_vec_big))
            .with_guessed_format()?
            .decode()?
            .to_rgba8();
        let img_sub = ImageReader::new(Cursor::new(img_vec_sub))
            .with_guessed_format()?
            .decode()?
            .to_rgba8();
        let x = text3.parse::<i64>()?;
        ...
        for i in 0..img_sub.width() {
            for j in 0..img_sub.height() {
                ...
                let jj = y + j as i64;
                if ii >= 0
                    && (ii as u32) < img_big.width()
                    && jj >= 0
                    && (jj as u32) < img_big.height()
                {
    ...
    add_fun(vec!["图片模糊", "图像模糊"], |self_t, params| {
        ...
        let img = ImageReader::new(Cursor::new(img_vec))
            .with_guessed_format()?
            .decode()?
            .to_rgba8();
    ...
    add_fun(vec!["GIF合成"], |self_t, params| {
        ...
        for it in img_arr_str {
            ...
            let img = ImageReader::new(Cursor::new(img_bin))
                ...
                .decode()?
                .to_rgba8();
            let fm = image::Frame::from_parts(
                img,
                0,
                0,
                image::Delay::from_saturating_duration(Duration::from_millis(delay)),
            );
            frame_vec.push(fm);
        }
        ...
        for frame in webp_frames {
            ...
            match frame.get_layout() {
                webp::PixelLayout::Rgb => {
                    let img_buf: ImageBuffer<image::Rgb<u8>, Vec<_>> = ImageBuffer::from_vec(
                        frame.width(),
                        frame.height(),
                ...
                webp::PixelLayout::Rgba => {
                    let img_buf: ImageBuffer<image::Rgba<u8>, Vec<_>> = ImageBuffer::from_vec(
                        frame.width(),
                        frame.height(),
                        frame.get_image().to_vec(),
                    )
    ...
    add_fun(vec!["WEBP合成"], |self_t, params| {
        ...
        for it in img_arr_str {
            let img_bin = RedLang::parse_bin(it)?;
            let img_buf = ImageReader::new(Cursor::new(img_bin))
                .with_guessed_format()?
                .decode()?
                .to_rgba8();
    ...
    add_fun(vec!["图片变圆", "图像变圆"], |self_t, params| {
        let text1 = self_t.get_param(params, 0)?;
        let img_vec = RedLang::parse_bin(&text1)?;
        let mut img = ImageReader::new(Cursor::new(img_vec))
            .with_guessed_format()?
            .decode()?
            .to_rgba8();
        let width = img.width();
    ...
    add_fun(vec!["创建图片", "创建图像"], |self_t, params| {
        ...
        let mut img: ImageBuffer<Rgba<u8>, Vec<u8>> =
            ImageBuffer::new(image_width, image_height as u32);
    ...
    add_fun(
        ...
        |self_t, params| {
            ...
            let img_vec = RedLang::parse_bin(&text1)?;
            let theta = text2.parse::<f32>()? / 360.0 * (2.0 * std::f32::consts::PI);
            let img_sub = ImageReader::new(Cursor::new(img_vec))
                .with_guessed_format()?
                .decode()?
                .to_rgba8();
            ...
            let vec1_t = (
                vec1.0 * cos_val - vec1.1 * sin_val,
                vec1.0 * sin_val + vec1.1 * cos_val,
            );
            let vec2_t = (
                vec2.0 * cos_val - vec2.1 * sin_val,
                vec2.0 * sin_val + vec2.1 * cos_val,
            );
            ...
            image::imageops::overlay(
                &mut img_big,
                &img_sub,
                ((max_wh - img_width) / 2).into(),
                ((max_wh - img_height) / 2).into(),
            );
            let mut img_out_t =
                rotate_about_center(&img_big, theta, Interpolation::Bilinear, Rgba([0, 0, 0, 0]));
            let img_out = image::imageops::crop(
                &mut img_out_t,
                ((max_wh - max_width * 2) / 2).into(),
                ((max_wh - max_height * 2) / 2).into(),
                max_width * 2,
                max_height * 2,
            )
            .to_image();
    ...
    add_fun(vec!["命令行"], |self_t, params| {
        ...
        let mut output_str = if cfg!(target_os = "windows") {
    ...
    add_fun(vec!["运行SQL"], |self_t, params| {
        ...
        return Ok(Some(
            self_t.build_arr(vec.iter().map(AsRef::as_ref).collect()),
        ));
    });
    add_fun(vec!["定义持久常量"], |self_t, params| {
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS CONST_TABLE (KEY TEXT PRIMARY KEY,VALUE TEXT);",
            [],
        )?;
        ...
        conn.execute(
            "REPLACE INTO CONST_TABLE (KEY,VALUE) VALUES (?,?)",
            [key, value],
        )?;
        return Ok(Some("".to_string()));
    });
    add_fun(vec!["持久常量"], |self_t, params| {
        ...
        let ret_rst: Result<String, rusqlite::Error> = conn.query_row(
            "SELECT VALUE FROM CONST_TABLE WHERE KEY = ?",
            [key],
            |row| row.get(0),
        );
        let ret_str;
            ...
            fn get_params(tokens: &Vec<String>) -> HashMap<String, (Option<i64>, Option<i64>)> {
                ...
                for index in 0..tokens.len() {
                        ...
                        || it == "f"
                    {
                        ret_map.insert(
                            it.to_owned(),
                            (get_left(index, tokens), get_right(index, &tokens)),
                        );
                    }
                }
                return ret_map;
    ...
    add_fun(vec!["补位"], |self_t, params| {
        ...
        if num_right_count <= num_format_right_count {
            ...
            out_str_num
                .push_str(&num[num_left_count + 1..num_left_count + num_format_right_count + 1]);
        }
        ...
        if params.len() > 1 {
            ...
            for i in 0..arr.len() - 1 {
                for j in i + 1..arr.len() {
                    let ret_str = self_t
                        .call_fun(&[func.clone(), arr[i].to_owned(), arr[j].to_owned()], true)?;
                    if ret_str != "真" {
...
"#;
        ...
        let content_rst = base64::Engine::decode(
            &base64::engine::GeneralPurpose::new(
                &base64::alphabet::STANDARD,
                base64::engine::general_purpose::PAD,
            ),
...
"#;
    ...
    add_fun(vec!["正则替换"], |self_t, params| {
        ...
        let re_obj = Regex::new(&re).unwrap();
        let out = re_obj.replace_all(&text, out_text).to_string();
        Ok(Some(out))
    });
}

pub fn do_json_parse(
...
fn do_json_arr(
    ...
    Ok(RedLang::build_arr_with_uid(
        self_uid,
        ret_str.iter().map(AsRef::as_ref).collect(),
    ))
}
