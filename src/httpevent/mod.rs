...
pub fn do_http_event(req:&hyper::Request<hyper::Body>) -> Result<hyper::Response<hyper::Body>, Box<dyn std::error::Error>> {
     // 获取pkg_name和pkg_key
    ...
    let pkg_name_t = urlencoding::decode(&pkg_name)?;
    let msg = urlencoding::decode(&pkg_key)?;
    ...
    for i in 0..script_json.as_array().ok_or("script.json文件不是数组格式")?.len(){
        ...
        if cffs == "网络触发" && pkg_name == pkg_name_t && crate::cqevent::is_key_match(&mut rl,&ppfs,keyword,&msg)? {
            ...
            let mut res:hyper::Response<hyper::Body> = hyper::Response::new(hyper::Body::from(rl_ret));
